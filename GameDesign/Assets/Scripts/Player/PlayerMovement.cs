...
public class PlayerMovement : MonoBehaviour
{
	...
	private void Update()
	{
		...
		GameProgress.Timer += Time.deltaTime;
		#endregion
		...
		if (Input.GetKeyDown(Controls.Get(InputAction.Bomb)))
			OnBombInput();
		...
		if (!IsDashing && !IsJumping)
		{
            ...
            if (groundCollider == null)
                groundCollider = Physics2D.OverlapBox
					(_groundCheckPoint.position, _groundCheckSize, 0,
	...
	public void OnBombInput()
	{
		if (_bombCD || !GameProgress.HasBomb)
			return;

		Instantiate(_bomb, transform.parent).transform.position = transform.position;
		StartCoroutine(BombCD(0.2f));
	}
	...
	private IEnumerator BombCD(float duration)
	{
		_bombCD = true;
        yield return new WaitForSecondsRealtime(duration);
        _bombCD = false;
